...
/src-tauri            Rust 后端
  ...
  /pty                PTY 会话管理、输入仲裁
  /drivers            ToolDriver: claude.rs / codex.rs / opencode.rs + sidecar 解析 + open_surfaces
...
trait ToolDriver {
  fn id(&self) -> Tool;                               // Claude | Codex | OpenCode
...
// OpenCode: 本机 server 的会话页;Claude: 无会话级 app-link → 降级 Editor(worktree)
```

各家命令映射:
- **Claude**:`claude` (+ `--add-dir <read_dirs>`,resume `--resume <id>`);sidecar=tail `~/.claude/projects/<编码cwd>/*.jsonl`。lead 首选(多目录 + subagents 强)。
