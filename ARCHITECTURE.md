...
每个会话切片同时开两条通道,指向**同一个原生会话**,互不打架:

- **交互通道(PTY,双向)**:在 worktree 的 cwd 下起原生 CLI 的交互式 TUI,xterm.js 渲染。**这是双向的,不是只读镜像**:xterm.js `onData` 捕获键盘 → `pty.write` 写入 stdin,人可直接打字、回答审批 `[y/N]`、Esc/Ctrl-C 打断、用斜杠命令——全部原生交互。程序(coordinator/thread bus)也可往同一 stdin 注入 prompt 实现"唤醒"。
  - **输入仲裁**:人正在打字时不注入,程序注入做成排队 prompt,避免抢 stdin。
- **数据通道(旁路,无需解析终端)**:
  - Claude Code → tail `~/.claude/projects/<编码cwd>/*.jsonl`
  - Codex → tail `~/.codex/sessions/.../*.jsonl`(rollout)
  - OpenCode → 订阅本机 `opencode serve` 的 `/event` SSE(最干净)

数据通道用于:跨仓 git diff 聚合、黑板更新、进度/状态、轻量编排触发。

> 注:`opencode serve` 是**本机 localhost 进程**,不是团队后端,不违背"无服务端"。

### 4.1 OpenCode 一等公民接入(三家里最 API-first,优先级与 Claude/Codex 同等)

OpenCode 走 API 路径而非 tail 日志,集成反而最干净:
...
- **cwd**:用 `--dir <worktree>` 指定工作目录,逐会话精确落到各自 worktree(规避"切会话不换 cwd"的已知问题 #6697)。
- **看 + 控**:TUI(Bubble Tea)走 PTY 嵌入观看;数据通道直接订阅 `/event` SSE(三家里唯一真 API)。TUI 本就是 server 客户端,可 attach 到独立 `opencode serve` 避免每次冷启动。
...
PTY 双向,产品在其四周叠交互层。开发按此实现。

**焦点模型**
- 点击面板即聚焦,聚焦态有可见描边 + "typing here" 标记;同一时刻仅一个面板拥有键盘焦点。
- 聚焦后所有按键默认进该会话 PTY 的 stdin。

**键位归属(必须定死,避免与 TUI 冲突)**
...
| 产品保留 | `⌘[` / `⌘]` | 上/下一个面板 |
| 产品保留 | `⌘1..9` | 跳到第 N 个会话 |
| 透传 TUI | `Esc` / `^C` | 中断 / 停止(头部按钮等价写入控制序列) |
| 透传 TUI | 其余全部(`/`、`↑↓` 历史、`Tab`、普通字符) | 原样进 PTY |

原则:**只截 ⌘ 前缀的少数键,其余一律透传**。

**审批流(从数据通道读 → 往 stdin 回写)**
- 来源:旁路通道(jsonl / rollout / SSE)识别审批/权限请求事件。**这是工具自身的权限习惯(用户在自己 CLI 里配的),Weft 尊重并透传,非 Weft 新增的 gate。**
- 呈现:面板转 warning 态 + 快捷条 `Approve·y` / `Deny·n` / `Always`。
- 动作:按钮 = 往 PTY 写 `y`/`n`(等同人敲);人也可直接在 TUI 里答。
- `Always`:产品侧记录本 thread 的放行规则,非工具原生。

**注入仲裁(状态机)** —— 程序(thread bus / coordinator)发往会话的消息不直接写 stdin,走队列:

```
idle ──消息到达──▶ queued ──(人点 Inject | 回合结束自动)──▶ injecting ──▶ idle
                    └──(人点 Hold)──▶ held ──(手动 release)──▶ queued
```

- 规则:人正在打字(stdin 有未提交输入)或 agent 回合进行中 → 不自动注入,停在 queued/banner。
- 方式:作为一个新 user turn,bracketed paste 整块写入,不逐字符。

**composer 行为**
- 短回复:直接在终端敲 + 回车。
- 长/多行/粘贴:用 composer,`Send`(⌘↵)用 bracketed paste 一次性写入 PTY,规避 TUI 中途渲染/换行问题。
- `@` 插入文件路径(产品文件选择器 → 写入路径文本);附件按各工具能力处理。

**会话状态枚举**:`starting | running | waiting-input | waiting-approval | injecting | paused | exited`。来源 = 数据通道事件 + PTY 存活探测;驱动头部 chip 与列表标识。
...
**交互界面(surface)与观测(observation)解耦**:`session.surface ∈ {embedded-pty | external-app | external-web}`,但无论哪种,Weft **始终经旁路通道观测同一会话**,diff/状态/bus 实时同步——深链跳走不致盲。

ToolDriver 暴露 `open_surfaces(session)`,面板给 "Open in…" 菜单,按工具能力出项:

- **Codex → 原生 app 深链**:`codex://threads/<nativeSessionId>`(UUID 就是我们抓的 native id,打开同一本地会话)。best-effort:契约未完全稳定、archived 会静默失败,需兜底。
- **OpenCode → 本机 web UI** 的该会话页。
- **Claude → 无会话级 app-link**,降级为"在 IDE 打开该 worktree"。

实现要点:跳外部 app 时**脱挂/暂停该会话 PTY**(别两头驱动),仅保留观测;回来再 re-attach(resume)。
...
- **机制 → 重隐藏 + inspect 入口**:worktree、PTY、MCP bus、`--add-dir`/`CODEX_HOME`、jsonl/rollout 旁路。
...
机制 → 产品词映射:worktree→"隔离的工作副本"(⋯里看路径/开终端);分支→只在交付时露出;PTY/TUI→"会话";thread bus/MCP→"任务间消息/handoff";sidecar/jsonl→"活动"(调试进 Inspect 高级);add-dir/只读挂载→"这个任务可读 X";物化→点"开始"时自然发生。
...
4. **按方向懒物化执行**(替代你现在手动开会话):
   ...
   - 注入团队 skills + PLAN.md → 起该方向指定工具的会话(可异构);TUI 嵌终端实时看,旁路通道聚合。
5. **收尾**:按仓/按 worktree 分支分别看 diff → 各自 PR/合并 → 清理 worktree。
6. **回流 CLI**:每个会话本就是原生会话,用户也可在终端 `claude --resume` / `codex resume` / opencode `/resume` 在**同一 worktree cwd** 接着干。
...
- **生命周期**:thread 有 `active|paused|archived`;空闲 thread 不常驻 PTY/进程,按需唤起,控制资源。
...
- **终端**:前端 xterm.js;PTY 用 Rust 侧 `portable-pty`(Tauri)或 Node 侧 `node-pty`(Web)。
- **TUI 渲染注意**:Claude Code 用 Ink、OpenCode 用 Bubble Tea,都是高频重绘的 TUI,直接转发会闪。需做**输出批处理/合帧管线**(Codeman 的做法:PTY → ~16ms 批 → 合适换行处理 → 传输 → 客户端 rAF → xterm.js)。
- **状态库**:SQLite(工作区/会话/worktree 映射)。
...
| 假设 | 评级 | 依据 | 风险/缓解 |
|---|---|---|---|
| 原生 TUI 嵌入 PTY 实时观看 | 🟢 已验证 | Codeman(Claude+OpenCode, PTY→SSE→xterm.js)、Claude Code Bridge(Nova, node-pty+xterm.js)、Claude Sidebar(Obsidian, 可切 Claude/Codex/OpenCode/Gemini)、Claude Code Remote 均已实现 | TUI 高频重绘会闪 → 用合帧/批处理管线(已有现成做法) |
...
- **TUI 闪烁/渲染**:合帧管线;必要时对个别工具退化为"结构化流自渲染"。
- **审批在 TUI 内、不在数据通道**:MVP 接受"审批各自 TUI 完成";若要工作区级统一拦截,需从日志读审批事件或对该工具改 headless。
...
**结论**:每块积木都被验证过,但"**逻辑多仓工作区 + worktree 物化 + 异构工具 + 分层 skills 下发 + 嵌入式 TUI**"这个组合,目前**无人做全**。可行性 OK,差异化清晰。
...
3. 各方向起一个原生会话,**三家(Claude Code / Codex / OpenCode)同为第一批**异构打通,TUI 嵌终端实时显示。
4. 旁路通道按仓展示 diff。
5. 关闭后能在终端 `claude --resume` / `codex resume` 在同一 worktree cwd 续上。

**里程碑建议**
- M1:PTY + xterm.js 跑通单个工具 TUI(含合帧),能创建/resume。
