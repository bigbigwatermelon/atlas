...
保留开发者母语英文词:`scope` / `brief` / `worker` / `lead` / `curator` / `PR` / `diff` / `headless`。
...
| `src/board/WorkspaceHome.tsx` / `WorkspaceKanban.tsx` | 工作区看板 | 两级;Workspace 总览 issues,Issue 看板展示 directions |
...
| `src/session/SessionView.tsx` `Transcript` `DiffPanel` `DiffView` | 会话工作台 | chat focus 环 + 审批条 + 注入 banner + 按仓 diff + 受保护合并 + Inspect 逃生舱 |
| `src/board/RepoMapView.tsx` `RepoGraph.tsx` | 仓库地图 | 依赖图(青色边)+ 仓 profile |
...
5. **跨仓地图前置**:Repo Profile + 依赖图作为 scope 分解燃料,不把它塞进任何单仓。
6. **机制隐入 Inspect**:worktree/headless engine/MCP/add-dir 进逃生舱;失败可读 + 就地真路径/开终端。
...
- **M-D · 信任仪表盘**:ThreadBoard 卡改 acceptance 信号 + 溯源;Workspace 看板保留按阶段总览。
...
- chat timeline 在明暗两主题下都使用同一套语义 token,代码和工具输出区域保持高对比。
...
- **响应式(窄窗)**:< ~1100px 会话 diff 堆叠到 chat timeline 下方(不再并排挤压);< ~940px 仓库地图改单列、顶栏收起搜索文案。Tauri 窗口可拖窄,需保这套降级。
- chat timeline、composer、approval bar 和 diff chrome 都随主题切换;代码块可保持更深一档的 surface。
...
### 7.3 卡片左对齐(易踩的坑)
- `<button>` 默认 `text-align: center`。**所有卡片/列表/菜单类按钮必须显式 `text-align: left`**(nav 项、issue 行、看板卡、命令面板项、派发行…),否则带 `flex:1`/`grow` 的文本会在容器里居中,编号/标题左缘对不齐。仅纯动作按钮(`.btn`、分段 tab)保持居中。
